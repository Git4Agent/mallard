...
- Sidebar remains resizable at its existing limits; three 27px actions must fit at the minimum width.
- Branch selector remains visible at the page header while scrolling.

## Components and behavior

- Header: local alias/repository name plus “history”; compact canonical path beneath; shared repository name appears only when aliased.
...
- Thread cards: title, date range, recorded SHA, non-color-only confidence label, and two explicit launch buttons.
- Non-Git: same header and a flat “Codex threads” list; do not render fake Git controls.
- Unmapped: separate section with a plain-language reason per thread.
